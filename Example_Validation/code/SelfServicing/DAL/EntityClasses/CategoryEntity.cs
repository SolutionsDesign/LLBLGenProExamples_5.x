...
namespace SD.LLBLGen.Pro.Examples.EntityClasses
{
		...
		// __LLBLGENPRO_USER_CODE_REGION_END	
	{
		...
		private SD.LLBLGen.Pro.Examples.CollectionClasses.SupplierCollection _supplierCollectionViaProduct;
		private bool	_alwaysFetchSupplierCollectionViaProduct, _alreadyFetchedSupplierCollectionViaProduct;
		...
		protected override void PerformPostReadXmlFixups()
		{
			...
			_alreadyFetchedSupplierCollectionViaProduct = (_supplierCollectionViaProduct.Count > 0);
		}
		...
		internal static RelationCollection GetRelationsForField(string fieldName)
		{
			...
			switch(fieldName)
			{
				case "Products":
					...
					break;
				case "SupplierCollectionViaProduct":
		...
		protected override void GetObjectData(SerializationInfo info, StreamingContext context)
		{
			...
			info.AddValue("_supplierCollectionViaProduct", (!this.MarkedForDeletion?_supplierCollectionViaProduct:null));
			info.AddValue("_alwaysFetchSupplierCollectionViaProduct", _alwaysFetchSupplierCollectionViaProduct);
			info.AddValue("_alreadyFetchedSupplierCollectionViaProduct", _alreadyFetchedSupplierCollectionViaProduct);
		...
		protected override void SetRelatedEntityProperty(string propertyName, IEntityCore entity)
		{
			switch(propertyName)
			{
				case "Products":
					...
					break;
				case "SupplierCollectionViaProduct":
					_alreadyFetchedSupplierCollectionViaProduct = true;
					if(entity!=null)
					{
						this.SupplierCollectionViaProduct.Add((SupplierEntity)entity);
					}
					break;
				default:
		...
		public SD.LLBLGen.Pro.Examples.CollectionClasses.SupplierCollection GetMultiSupplierCollectionViaProduct(bool forceFetch)
		{
			return GetMultiSupplierCollectionViaProduct(forceFetch, _supplierCollectionViaProduct.EntityFactoryToUse);
		}
		...
		/// <returns>Filled collection with all related entities of the type constructed by the passed in entity factory</returns>
		public SD.LLBLGen.Pro.Examples.CollectionClasses.SupplierCollection GetMultiSupplierCollectionViaProduct(bool forceFetch, IEntityFactory entityFactoryToUse)
		{
 			if( ( !_alreadyFetchedSupplierCollectionViaProduct || forceFetch || _alwaysFetchSupplierCollectionViaProduct) && !this.IsSerializing && !this.IsDeserializing && !this.InDesignMode)
			{
				AddToTransactionIfNecessary(_supplierCollectionViaProduct);
				...
				_supplierCollectionViaProduct.SuppressClearInGetMulti=!forceFetch;
				_supplierCollectionViaProduct.EntityFactoryToUse = entityFactoryToUse;
				_supplierCollectionViaProduct.GetMulti(filter, GetRelationsForField("SupplierCollectionViaProduct"));
				_supplierCollectionViaProduct.SuppressClearInGetMulti=false;
				_alreadyFetchedSupplierCollectionViaProduct = true;
			}
			return _supplierCollectionViaProduct;
		}

		/// <summary> Sets the collection parameters for the collection for 'SupplierCollectionViaProduct'. These settings will be taken into account
		/// when the property SupplierCollectionViaProduct is requested or GetMultiSupplierCollectionViaProduct is called.</summary>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return. When set to 0, this parameter is ignored</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified (null), no sorting is applied.</param>
		public virtual void SetCollectionParametersSupplierCollectionViaProduct(long maxNumberOfItemsToReturn, ISortExpression sortClauses)
		{
			_supplierCollectionViaProduct.SortClauses=sortClauses;
			_supplierCollectionViaProduct.MaxNumberOfItemsToReturn=maxNumberOfItemsToReturn;
		}
		...
		protected override Dictionary<string, object> GetRelatedData()
		{
			...
			toReturn.Add("SupplierCollectionViaProduct", _supplierCollectionViaProduct);
			return toReturn;
		}
		...
		private void InitClassMembers()
		{
			...
			_supplierCollectionViaProduct = new SD.LLBLGen.Pro.Examples.CollectionClasses.SupplierCollection();
		...
		{
			get { return new PrefetchPathElement(new SD.LLBLGen.Pro.Examples.CollectionClasses.ProductCollection(), (IEntityRelation)GetRelationsForField("Products")[0], (int)SD.LLBLGen.Pro.Examples.EntityType.CategoryEntity, (int)SD.LLBLGen.Pro.Examples.EntityType.ProductEntity, 0, null, null, null, "Products", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.OneToMany); }
		}
		...
		/// <returns>Ready to use IPrefetchPathElement implementation.</returns>
		public static IPrefetchPathElement PrefetchPathSupplierCollectionViaProduct
		{
			get
			{
				...
				return new PrefetchPathElement(new SD.LLBLGen.Pro.Examples.CollectionClasses.SupplierCollection(), intermediateRelation,	(int)SD.LLBLGen.Pro.Examples.EntityType.CategoryEntity, (int)SD.LLBLGen.Pro.Examples.EntityType.SupplierEntity, 0, null, null, GetRelationsForField("SupplierCollectionViaProduct"), "SupplierCollectionViaProduct", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.ManyToMany);
			}
		}
		...
		/// </summary>
		/// <remarks>This property is added for databinding conveniance, however it is recommeded to use the method 'GetMultiSupplierCollectionViaProduct()', because 
		/// this property is rather expensive and a method tells the user to cache the result when it has to be used more than once in the same scope.</remarks>
		public virtual SD.LLBLGen.Pro.Examples.CollectionClasses.SupplierCollection SupplierCollectionViaProduct
		{
			get { return GetMultiSupplierCollectionViaProduct(false); }
		}

		/// <summary> Gets / sets the lazy loading flag for SupplierCollectionViaProduct. When set to true, SupplierCollectionViaProduct is always refetched from the 
		/// persistent storage. When set to false, the data is only fetched the first time SupplierCollectionViaProduct is accessed. You can always execute a forced fetch by calling GetMultiSupplierCollectionViaProduct(true).</summary>
		[Browsable(false)]
		public bool AlwaysFetchSupplierCollectionViaProduct
		{
			get	{ return _alwaysFetchSupplierCollectionViaProduct; }
			set	{ _alwaysFetchSupplierCollectionViaProduct = value; }
		}
				
		/// <summary>Gets / Sets the lazy loading flag if the property SupplierCollectionViaProduct already has been fetched. Setting this property to false when SupplierCollectionViaProduct has been fetched
		/// will clear the SupplierCollectionViaProduct collection well. Setting this property to true while SupplierCollectionViaProduct hasn't been fetched disables lazy loading for SupplierCollectionViaProduct</summary>
		[Browsable(false)]
		public bool AlreadyFetchedSupplierCollectionViaProduct
		{
			get { return _alreadyFetchedSupplierCollectionViaProduct;}
			set 
			{
				if(_alreadyFetchedSupplierCollectionViaProduct && !value && (_supplierCollectionViaProduct != null))
				{
					_supplierCollectionViaProduct.Clear();
				}
				_alreadyFetchedSupplierCollectionViaProduct = value;
			}
		}
		...
		{ 
			get { return (int)SD.LLBLGen.Pro.Examples.EntityType.CategoryEntity; }
		}
