...
namespace SD.LLBLGen.Pro.Examples.DaoClasses
{
	...
	public partial class ProductDAO : CommonDaoBase
	{
		...
		public bool GetMulti(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IPredicateExpression filter, IEntity categoryInstance, IEntity supplierInstance, int pageNumber, int pageSize)
		{
			...
			IEntityFields fieldsToReturn = EntityFieldsFactory.CreateEntityFieldsObject(SD.LLBLGen.Pro.Examples.EntityType.ProductEntity);
		...
		public bool GetMultiUsingOrderCollectionViaOrderDetails(ITransaction containingTransaction, IEntityCollection collectionToFill, long maxNumberOfItemsToReturn, ISortExpression sortClauses, IEntityFactory entityFactoryToUse, IEntity orderInstance, IPrefetchPath prefetchPathToUse, int pageNumber, int pageSize)
		{
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="categoryInstance">CategoryEntity instance to use as a filter for the ProductEntity objects to delete</param>
		/// <param name="supplierInstance">SupplierEntity instance to use as a filter for the ProductEntity objects to delete</param>
		...
		{
			IEntityFields fields = EntityFieldsFactory.CreateEntityFieldsObject(SD.LLBLGen.Pro.Examples.EntityType.ProductEntity);
		...
		/// <param name="containingTransaction">A containing transaction, if caller is added to a transaction, or null if not.</param>
		/// <param name="categoryInstance">CategoryEntity instance to use as a filter for the ProductEntity objects to update</param>
		/// <param name="supplierInstance">SupplierEntity instance to use as a filter for the ProductEntity objects to update</param>
		/// <returns>Amount of entities affected, if the used persistent storage has rowcounting enabled.</returns>
		...
		/// <param name="supplierInstance">SupplierEntity instance to use as a filter for the ProductEntity objects</param>
		...
		private IPredicateExpression CreateFilterUsingForeignKeys(IEntity categoryInstance, IEntity supplierInstance, IEntityFields fieldsToReturn)
		{
			...
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)ProductFieldIndex.CategoryId], ComparisonOperator.Equal, ((CategoryEntity)categoryInstance).CategoryId));
			}
			if(supplierInstance != null)
			{
				selectFilter.Add(new FieldCompareValuePredicate(fieldsToReturn[(int)ProductFieldIndex.SupplierId], ComparisonOperator.Equal, ((SupplierEntity)supplierInstance).SupplierId));
			}
			return selectFilter;
		}
