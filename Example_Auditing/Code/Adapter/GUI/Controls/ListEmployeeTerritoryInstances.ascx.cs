...
public partial class Controls_ListEmployeeTerritoryInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.EmployeeTerritoryEntity + additionalFiltersAsString);
	}
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		EmployeeTerritoryEntity selectedEntity = (EmployeeTerritoryEntity)_EmployeeTerritoryDS.EntityCollection[index];

		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&EmployeeId={0}", selectedEntity.EmployeeId);		pkFieldsAndValues.AppendFormat("&TerritoryId={0}", selectedEntity.TerritoryId);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.EmployeeTerritoryEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.EmployeeTerritoryEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
	...
	public void SetContainingEntity(IEntity2 containingEntity, string name)
	{
		switch(containingEntity.LLBLGenProEntityName)
		{
			case "EmployeeEntity":
				switch(name)
				{
					case "EmployeeTerritories":
						_EmployeeTerritoryDS.FilterToUse = ((EmployeeEntity)containingEntity).GetRelationInfoEmployeeTerritories();
						ViewState["additionalFilters"] = "&EmployeeId=" + 
								((EmployeeEntity)containingEntity).EmployeeId +
								"&20FkField=EmployeeId";
						break;
					default:
						break;
				}
			...
			case "TerritoryEntity":
				...
				{
					case "EmployeeTerritories":
						_EmployeeTerritoryDS.FilterToUse = ((TerritoryEntity)containingEntity).GetRelationInfoEmployeeTerritories();
						ViewState["additionalFilters"] = "&TerritoryId=" + 
								((TerritoryEntity)containingEntity).TerritoryId +
								"&20FkField=TerritoryId";
						break;
					default:
						break;
				}
	...
	{
		set { _EmployeeTerritoryDS.FilterToUse = value; }
	}
