...
public partial class Controls_ListAuditInfoInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.AuditInfoEntity + additionalFiltersAsString);
	}
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		AuditInfoEntity selectedEntity = (AuditInfoEntity)_AuditInfoDS.EntityCollection[index];
		...
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.AuditInfoEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.AuditInfoEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
	...
	public void SetContainingEntity(IEntity2 containingEntity, string name)
	{
		switch(containingEntity.LLBLGenProEntityName)
		{
			...
			case "UserEntity":
				...
				{
					case "AuditInfo":
						_AuditInfoDS.FilterToUse = ((UserEntity)containingEntity).GetRelationInfoAuditInfo();
						ViewState["additionalFilters"] = "&UserId=" + 
								((UserEntity)containingEntity).UserId +
								"&20FkField=UserId";
						break;
					default:
						break;
	...
	{
		set { _AuditInfoDS.FilterToUse = value; }
	}
