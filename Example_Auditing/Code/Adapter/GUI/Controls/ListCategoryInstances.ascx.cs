...
public partial class Controls_ListCategoryInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.CategoryEntity + additionalFiltersAsString);
	}
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		CategoryEntity selectedEntity = (CategoryEntity)_CategoryDS.EntityCollection[index];

		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&CategoryId={0}", selectedEntity.CategoryId);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.CategoryEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.CategoryEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
			...
			case "SupplierEntity":
				switch(name)
				{
					case "CategoriesCollectionViaProducts":
						_CategoryDS.FilterToUse = ((SupplierEntity)containingEntity).GetRelationInfoCategoriesCollectionViaProducts();
						break;
					default:
						break;
				}
	...
	{
		set { _CategoryDS.FilterToUse = value; }
	}
