...
public partial class Controls_ListCustomerInstances : System.Web.UI.UserControl, IListControl
{
	...
	protected void btnAddNew_Click(object sender, EventArgs e)
	{
		...
		Response.Redirect("~/AddNew.aspx?EntityType=" + (int)EntityType.CustomerEntity + additionalFiltersAsString);
	}
	...
	protected void theGrid_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		CustomerEntity selectedEntity = (CustomerEntity)_CustomerDS.EntityCollection[index];

		StringBuilder pkFieldsAndValues = new StringBuilder();
		pkFieldsAndValues.AppendFormat("&CustomerId={0}", selectedEntity.CustomerId);
		switch(e.CommandName)
		{
			case "EditExisting":
				Response.Redirect("~/EditExisting.aspx?EntityType=" + (int)EntityType.CustomerEntity + pkFieldsAndValues.ToString());
				break;
			case "ViewExisting":
				Response.Redirect("~/ViewExisting.aspx?EntityType=" + (int)EntityType.CustomerEntity + pkFieldsAndValues.ToString());
				break;
		}
	}
	...
	public void SetContainingEntity(IEntity2 containingEntity, string name)
	{
		...
		{
			case "CustomerCustomerDemoEntity":
				switch(name)
				{
					default:
						break;
			...
			case "OrderEntity":
				...
				break;
			case "CustomerDemographyEntity":
				switch(name)
				{
					case "CustomersCollectionViaCustomerCustomerDemo":
						_CustomerDS.FilterToUse = ((CustomerDemographyEntity)containingEntity).GetRelationInfoCustomersCollectionViaCustomerCustomerDemo();
						break;
					default:
						break;
				}
			...
			case "EmployeeEntity":
				switch(name)
				{
					case "CustomersCollectionViaOrders":
						_CustomerDS.FilterToUse = ((EmployeeEntity)containingEntity).GetRelationInfoCustomersCollectionViaOrders();
						break;
					default:
						break;
				}
			...
			case "ShipperEntity":
				...
				{
					case "CustomersCollectionViaOrders":
						_CustomerDS.FilterToUse = ((ShipperEntity)containingEntity).GetRelationInfoCustomersCollectionViaOrders();
						break;
					default:
						break;
				}
	...
	{
		set { _CustomerDS.FilterToUse = value; }
	}
