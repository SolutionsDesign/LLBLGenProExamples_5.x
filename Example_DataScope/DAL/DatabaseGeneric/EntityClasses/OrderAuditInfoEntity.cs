...
namespace Northwind.DAL.EntityClasses
{
		...
		// __LLBLGENPRO_USER_CODE_REGION_END	
	{
		...
		{
			return InheritanceInfoProviderSingleton.GetInstance().CheckIfIsSubTypeOf("OrderAuditInfoEntity", ((Northwind.DAL.EntityType)typeOfEntity).ToString());
		}
		...
		{
			return EntityFactoryCache2.GetEntityFactory(typeof(OrderAuditInfoEntityFactory));
		}
#if !CF
		...
		{
			this.PerformDesetupSyncRelatedEntity( _order, new PropertyChangedEventHandler( OnOrderPropertyChanged ), "Order", Northwind.DAL.RelationClasses.StaticOrderAuditInfoRelations.OrderEntityUsingOrderIdStatic, true, signalRelatedEntity, "OrderAuditInfos", resetFKFields, new int[] { (int)OrderAuditInfoFieldIndex.OrderId } );
			_order = null;
		}
		...
		private void SetupSyncOrder(IEntityCore relatedEntity)
		{
			if(_order!=relatedEntity)
			{
				...
				this.PerformSetupSyncRelatedEntity( _order, new PropertyChangedEventHandler( OnOrderPropertyChanged ), "Order", Northwind.DAL.RelationClasses.StaticOrderAuditInfoRelations.OrderEntityUsingOrderIdStatic, true, new string[] {  } );
			}
		}
		...
		{
			get	{ return new PrefetchPathElement2(new EntityCollection(EntityFactoryCache2.GetEntityFactory(typeof(OrderEntityFactory))),	(IEntityRelation)GetRelationsForField("Order")[0], (int)Northwind.DAL.EntityType.OrderAuditInfoEntity, (int)Northwind.DAL.EntityType.OrderEntity, 0, null, null, null, null, "Order", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.ManyToOne); }
		}
		...
		{ 
			get { return (int)Northwind.DAL.EntityType.OrderAuditInfoEntity; }
		}
