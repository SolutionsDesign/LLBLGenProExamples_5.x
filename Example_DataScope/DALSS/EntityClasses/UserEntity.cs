...
namespace Northwind.SSDAL.EntityClasses
{
		...
		// __LLBLGENPRO_USER_CODE_REGION_END	
	{
		...
		private bool	_alwaysFetchAuditInfos, _alreadyFetchedAuditInfos;
		...
		{
			_auditInfos = (Northwind.SSDAL.CollectionClasses.AuditInfoCollection)info.GetValue("_auditInfos", typeof(Northwind.SSDAL.CollectionClasses.AuditInfoCollection));
			_alwaysFetchAuditInfos = info.GetBoolean("_alwaysFetchAuditInfos");
			_alreadyFetchedAuditInfos = info.GetBoolean("_alreadyFetchedAuditInfos");
		...
		{
			_alreadyFetchedAuditInfos = (_auditInfos.Count > 0);
		...
		{
			info.AddValue("_auditInfos", (!this.MarkedForDeletion?_auditInfos:null));
			info.AddValue("_alwaysFetchAuditInfos", _alwaysFetchAuditInfos);
			info.AddValue("_alreadyFetchedAuditInfos", _alreadyFetchedAuditInfos);
		...
		protected override void SetRelatedEntityProperty(string propertyName, IEntityCore entity)
		{
			...
			{
				case "AuditInfos":
					_alreadyFetchedAuditInfos = true;
					if(entity!=null)
					{
		...
		protected override void SetRelatedEntity(IEntityCore relatedEntity, string fieldName)
		{
			...
			{
				case "AuditInfos":
					_auditInfos.Add((AuditInfoEntity)relatedEntity);
					break;
				case "UserGroups":
		...
		protected override void UnsetRelatedEntity(IEntityCore relatedEntity, string fieldName, bool signalRelatedEntityManyToOne)
		{
			...
			{
				case "AuditInfos":
					this.PerformRelatedEntityRemoval(_auditInfos, relatedEntity, signalRelatedEntityManyToOne);
					break;
				case "UserGroups":
		...
		{
 			if( ( !_alreadyFetchedAuditInfos || forceFetch || _alwaysFetchAuditInfos) && !this.IsSerializing && !this.IsDeserializing && !this.InDesignMode)
			{
				AddToTransactionIfNecessary(_auditInfos);
				_auditInfos.SuppressClearInGetMulti=!forceFetch;
				_auditInfos.EntityFactoryToUse = entityFactoryToUse;
				_auditInfos.GetMultiManyToOne(null, this, filter);
				_auditInfos.SuppressClearInGetMulti=false;
				_alreadyFetchedAuditInfos = true;
			}
			return _auditInfos;
		}
		...
		public virtual Northwind.SSDAL.CollectionClasses.UserGroupCollection GetMultiUserGroups(bool forceFetch, IEntityFactory entityFactoryToUse, IPredicateExpression filter)
		{
 			if( ( !_alreadyFetchedUserGroups || forceFetch || _alwaysFetchUserGroups) && !this.IsSerializing && !this.IsDeserializing && !this.InDesignMode)
			{
				...
				_userGroups.GetMultiManyToOne(null, this, filter);
		...
		public static IPrefetchPathElement PrefetchPathAuditInfos
		{
			get { return new PrefetchPathElement(new Northwind.SSDAL.CollectionClasses.AuditInfoCollection(), (IEntityRelation)GetRelationsForField("AuditInfos")[0], (int)Northwind.SSDAL.EntityType.UserEntity, (int)Northwind.SSDAL.EntityType.AuditInfoEntity, 0, null, null, null, "AuditInfos", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.OneToMany); }
		}
		...
		{
			get { return new PrefetchPathElement(new Northwind.SSDAL.CollectionClasses.UserGroupCollection(), (IEntityRelation)GetRelationsForField("UserGroups")[0], (int)Northwind.SSDAL.EntityType.UserEntity, (int)Northwind.SSDAL.EntityType.UserGroupEntity, 0, null, null, null, "UserGroups", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.OneToMany); }
		}
		...
		/// </summary>
		/// <remarks>This property is added for databinding conveniance, however it is recommeded to use the method 'GetMultiAuditInfos()', because 
		/// this property is rather expensive and a method tells the user to cache the result when it has to be used more than once in the same scope.</remarks>
		public virtual Northwind.SSDAL.CollectionClasses.AuditInfoCollection AuditInfos
		{
			get	{ return GetMultiAuditInfos(false); }
		}
		...
		{
			get	{ return _alwaysFetchAuditInfos; }
			set	{ _alwaysFetchAuditInfos = value; }	
		}		
		...
		{
			get { return _alreadyFetchedAuditInfos;}
			set 
			{
				if(_alreadyFetchedAuditInfos && !value && (_auditInfos != null))
				{
					_auditInfos.Clear();
				}
				_alreadyFetchedAuditInfos = value;
			}
		}
		...
		{ 
			get { return (int)Northwind.SSDAL.EntityType.UserEntity; }
		}
